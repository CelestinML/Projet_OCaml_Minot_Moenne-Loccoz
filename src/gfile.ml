...
let export path gr =
  ...
  (* Formatage du fichier pour dot *)
  fprintf ff "digraph finite_state_machine {
    rankdir=LR;
    size=\"8,5\"
    node [shape = circle];\n";

  (* Pour chaque arc, on convertit le label en un format lisible pour que dot puisse afficher le graphe *)
  e_iter gr (fun id1 id2 lbl -> fprintf ff "LR_%d -> LR_%d [ label = \"SS(%s)\" ];\n" id1 id2 lbl);
